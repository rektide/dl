...
import type { ExecuteContext, FanIn, InputFlowExecutor } from "./types.ts";
...
export function createInputFlowExecutor(merge: FanIn<Repo> = fanIn): InputFlowExecutor {
  return async function* execute(
    ...
    for await (const input of inputs) {
      ...
      const candidateStreams = providers.map((provider) =>
        provider.candidates(input.value, ctx.providerRuntime),
      );
