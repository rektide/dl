...
/**
 * npm registry provider — resolves npm package names to their repository URL
 * via the npm registry API. Handles URL cleaning for git+/ssh:// prefixed URLs.
 */
export class NpmDevProvider extends RedirectRepo {
	name = "npm-registry"
	...
	extractIdentifier(input: string): string | undefined {
		...
		if (isUrl(trimmed)) {
			...
			if (
				parsed &&
				(parsed.host === "npmx.dev" ||
					parsed.host === "www.npmjs.com" ||
					parsed.host === "npmjs.com")
			) {
...
export const npmxDevProvider = new NpmDevProvider()
